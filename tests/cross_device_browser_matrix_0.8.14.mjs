...
const cells=[
  ...
  {id:'webkit-desktop',engine:'webkit',policy:'FAIL_CLOSED',expectedReason:'WEBKIT_CROSS_TAB_STORAGE_COHERENCE_UNCERTIFIED',context:{viewport:{width:1365,height:900},locale:'ko-KR'}},
  {id:'webkit-iphone14',engine:'webkit',policy:'FAIL_CLOSED',expectedReason:'WEBKIT_CROSS_TAB_STORAGE_COHERENCE_UNCERTIFIED',context:{...devices['iPhone 14'],locale:'ko-KR'}}
];
const HISTORICAL_UNSAFE={
  ...
  webkit_ios:{workflow_run:30752906868,observed:'RESPONSE_APPLIED,RESPONSE_APPLIED',cell:'webkit-iphone14'}
};
...
function attachErrors(p,label,errors){p.on('pageerror',e=>errors.push({label,type:'pageerror',text:String(e.message||e)}));p.on('console',m=>{if(m.type()==='error')errors.push({label,type:'console',text:m.text()})})}
...
async function runFailClosedCell(cell){
  ...
  try{
    ...
    return {id:cell.id,engine:cell.engine,required_policy:cell.policy,status:'PASS_FAIL_CLOSED',started_at:startedAt,completed_at:new Date().toISOString(),gate:result.gate,user_agent:result.user_agent,storage_keys:result.storage_keys,test_hooks_present:false,begin_present:false,message_present:result.visible_text.includes('과제를 시작하지 않았습니다'),errors};
...
try{
  ...
  diagnostics={
    ...
    webkit_ios:await diagnoseParentEngine('webkit-ios',webkit,{...devices['iPhone 14'],locale:'ko-KR'},HISTORICAL_UNSAFE.webkit_ios)
  };
  console.log(`CR0814_PARENT_DIAGNOSTICS firefox_dual=${diagnostics.firefox.dual_apply_count}/6 webkit_desktop_dual=${diagnostics.webkit_desktop.dual_apply_count}/6 webkit_ios_dual=${diagnostics.webkit_ios.dual_apply_count}/6 firefox_lock=${diagnostics.firefox.lock_probe.serialized} webkit_desktop_lock=${diagnostics.webkit_desktop.lock_probe.serialized} webkit_ios_lock=${diagnostics.webkit_ios.lock_probe.serialized}`);
...
const report={schema_version:'CR0814-CROSS-DEVICE-BROWSER-MATRIX-4',route:'participant-cognitive-mode-0.8.14.html',parent_diagnostic_route:'participant-cognitive-mode-0.8.13.html',matrix_kind:'PLAYWRIGHT_ENGINE_AND_DEVICE_EMULATION_WITH_CHROMIUM_ONLY_ACTIVE_POLICY_NOT_PHYSICAL_DEVICE',required_cells:cells.map(x=>({id:x.id,policy:x.policy,expected_reason:x.expectedReason||null})),passed_cells:passed,failed_cells:results.length-passed,active_execution_passed_cells:activePassed,fail_closed_passed_cells:failClosedPassed,active_race_iterations_passed:activeIterations,parent_concurrency_diagnostics:diagnostics,results,physical_device_certified:false,chromium_active_execution_certified_automated:true,firefox_active_execution_certified:false,webkit_active_execution_certified:false,result:passed===cells.length&&activePassed===2&&failClosedPassed===4&&activeIterations===8?'PASS_CONTROLLED_STAGING_BROWSER_POLICY_MATRIX':'FAIL_CONTROLLED_STAGING_BROWSER_POLICY_MATRIX'};
fs.mkdirSync(path.join(ROOT,'artifacts/0.8.14'),{recursive:true});fs.writeFileSync(path.join(ROOT,'artifacts/0.8.14/cross_device_browser_matrix.json'),JSON.stringify(report,null,2));if(report.result!=='PASS_CONTROLLED_STAGING_BROWSER_POLICY_MATRIX')throw new Error(`DEVICE_POLICY_MATRIX_INCOMPLETE:${passed}/${cells.length};active=${activePassed};blocked=${failClosedPassed};iterations=${activeIterations}`);console.log(`CR0814_CROSS_DEVICE_POLICY_MATRIX_PASS active_cells=${activePassed} fail_closed_cells=${failClosedPassed} active_races=${activeIterations} physical_device_certified=false active_policy=CHROMIUM_ONLY`);
