...
import asyncio, json, os, shutil, socket, threading, traceback
...
async def wait_input(page): await page.wait_for_function("typeof inputEnabled!=='undefined' && inputEnabled===true",timeout=12000)

async def select_keymap(page,keymap):
    loc=page.locator(f'input[name="keymap"][value="{keymap}"]')
    if await loc.count()==0: return False
    if await loc.is_visible(): await loc.check()
    else:
        await page.evaluate("k=>{const e=document.querySelector(`input[name=keymap][value='${k}']`);if(e){e.checked=true;e.dispatchEvent(new Event('change',{bubbles:true}));}}",keymap)
...
async def start_custom(page,keymap='speed'):
    ...
    await page.check('#consentCheck'); await page.check('input[name="mode"][value="custom"]'); await page.fill('#customScramble','R U')
    await select_keymap(page,keymap)
...
async def desktop_court(browser):
    ctx=await browser.new_context(accept_downloads=True,viewport={'width':1440,'height':900}); await ctx.route('**/*',block_remote); p=await ctx.new_page(); remote=[]
    ...
    s1=await snap(p); orbit_delta=max(abs(float(s1['camera'].get('pitch',0))-float(s0['camera'].get('pitch',0))),abs(float(s1['camera'].get('yaw',0))-float(s0['camera'].get('yaw',0))))
    evs=await p.evaluate("() => session.events.filter(e=>e.type==='camera_drag_end')"); accumulated=sum(abs(float(e.get('delta_pitch',0) or 0))+abs(float(e.get('delta_yaw',0) or 0)) for e in evs)
    ...
    drag_before=await p.evaluate("() => session.events.filter(e=>e.type==='move_accepted').length"); hit=await p.evaluate("() => projectedStickerHits.find(h=>h.front && h.screenCenter)"); drag_ok=False
    if hit:
    ...
    await p.click('button[data-move="x"]'); await p.wait_for_timeout(140); xev=await p.evaluate("() => session.events.filter(e=>e.type==='move_accepted').slice(-1)[0]"); check('whole_cube_rotation_control',bool(xev) and xev.get('move')=='x',xev)
    ...
    p.once('dialog',lambda d: asyncio.create_task(d.accept())); await p.click('#resetTrialButton'); await p.wait_for_timeout(180); reset_ev=await p.evaluate("() => session.events.some(e=>e.type==='trial_reset_applied')"); check('trial_reset_button',reset_ev)
    ...
    cams=[e for e in raw.get('events',[]) if str(e.get('type','')).startswith('camera_')]; complete=[e for e in cams if all(e.get(k) is not None for k in ('yaw','pitch','zoom','view_matrix','input_source'))]
    check('camera_telemetry_exact_logger',len(cams)>0 and len(complete)==len(cams),{'camera_events':len(cams),'complete':len(complete)})
    check('collector_external_network_blocked',len(remote)>=1,{'remote_requests_aborted':len(remote),'sample':remote[:3]})
...
async def mode_court(browser):
    ...
    ctx=await browser.new_context(viewport={'width':1440,'height':900}); await ctx.route('**/*',block_remote); p=await ctx.new_page(); await start_custom(p,'notation'); keymap=await p.evaluate("() => session.interface_settings.keymap_mode"); check('notation_keymap_selected',keymap=='notation',keymap); n0=await p.evaluate("() => session.events.filter(e=>e.type==='move_accepted').length"); await p.keyboard.press('KeyU'); await p.wait_for_timeout(130); n1=await p.evaluate("() => session.events.filter(e=>e.type==='move_accepted').length"); check('notation_keyboard_layout',n1>n0); await ctx.close()
...
async def mobile_court(browser):
    ...
    box=await p.locator('#cubeCanvas').bounding_box(); z0=float((await snap(p))['camera']['zoom']); cdp=await ctx.new_cdp_session(p); cx=box['x']+box['width']/2; cy=box['y']+box['height']/2
...
async def main():
    global BASE
    srv,port=server(); BASE=f'http://127.0.0.1:{port}/index.html?debug=1'; runtime_errors=[]
    try:
      async with async_playwright() as pw:
        exe=shutil.which('google-chrome') or shutil.which('chromium') or shutil.which('chromium-browser'); browser=await pw.chromium.launch(headless=True,executable_path=exe if exe else None,args=['--no-sandbox','--disable-dev-shm-usage'])
        for name,fn in [('desktop',desktop_court),('modes',mode_court),('mobile',mobile_court)]:
          try: await fn(browser)
          except Exception as e:
            runtime_errors.append({'court':name,'error':repr(e),'traceback':traceback.format_exc()}); check(name+'_court_runtime',False,repr(e))
        await browser.close()
    finally: srv.shutdown()
    result={'schema_version':'CR0105R11-EXACT-MONOLITH-BROWSER-2','status':'PASS' if all(x['pass'] for x in checks) and not runtime_errors else 'FAIL','check_count':len(checks),'pass_count':sum(x['pass'] for x in checks),'checks':checks,'runtime_errors':runtime_errors,'human_observations':0,'source_class':'ENGINEERING_BROWSER_DRY_RUN','production_network_performed':False}
