...
const EXPECTED={
  'archival-live-head':{
    ...
    construction:'INLINE_LIVE_PROBE_COMPATIBILITY_ENVELOPE_V1'
  },
  'final-0.8.13-head':{
    snapshot_sha256:'5fbf313d1a81bc7d94820da42a588e94cccba5aa14c287316cf548175ef82f83',
    ...
    construction:'FINAL_RUNTIME_COLLECTOR_ENVELOPE_HELPER_V1'
  }
}[runtimeClass];
...
function archivalEnvelope(snapshot,snapshotSha256){
  ...
  return {
    ...
    generated_at:'2026-08-02T11:00:30.000Z',trials:compatibilityTrials,
    data_submission:{
...
function finalRuntimeEnvelope(snapshot,snapshotSha256){
  ...
  return M.collectorEnvelopeFromSnapshot(snapshot,{
    generated_at:'2026-08-02T11:00:30.000Z',status:'engineering_synthetic_live_cert',
    synthetic_live_cert:true,exclude_from_human_cohort:true,immutable_snapshot_sha256:snapshotSha256
  });
}
...
const snapshotSha256=sha(snapshotText);
assertEqual(snapshotSha256,EXPECTED.snapshot_sha256,'SNAPSHOT_SHA256');
const envelope=runtimeClass==='archival-live-head'
  ?archivalEnvelope(snapshot,snapshotSha256)
  :finalRuntimeEnvelope(snapshot,snapshotSha256);
...
fs.writeFileSync(evidencePath,JSON.stringify({schema_version:'CR0814-RUNTIME-PINNED-ENVELOPE-RECONSTRUCTION-2',runtime_class:runtimeClass,expected:EXPECTED,observed,exact_runtime_pinned_bytes_reconstructed:true,exact_drive_stored_bytes_retrieved:false,result:runtimeClass==='archival-live-head'?'PASS_ARCHIVAL_LIVE_SUBMITTED_BYTE_RECONSTRUCTION':'PASS_FINAL_RUNTIME_COUNTERFACTUAL_RECONSTRUCTION'},null,2));
console.log(`CR0814_RUNTIME_PINNED_RECONSTRUCTION_PASS class=${runtimeClass} construction=${EXPECTED.construction} bytes=${observed.bytes} sha256=${observed.envelope_sha256} checksum=${observed.checksum_fnv1a32} embedded_snapshot_sha256=${observed.embedded_snapshot_sha256}`);
