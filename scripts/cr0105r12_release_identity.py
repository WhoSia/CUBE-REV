...
def main():
    ...
    pages_status=int(pages.get('http_status') or 0)
    host_status=int(host.get('http_status') or 0)
    checks['pages_metadata_resolved']=pages_status==200 and bool(pages.get('html_url'))
    ...
    checks['main_ref_unchanged']=os.environ.get('CR0105R12_MAIN_SHA')==BASE_MAIN
    ...
    out={
      'schema_version':'CR0105R12-PRELAUNCH-SEAL-1',
      ...
      'pages':{'http_status':pages_status,'html_url':pages.get('html_url'),'source':pages.get('source'),'build_type':pages.get('build_type')},
      'public_host_fetch':host,'browser_check_count':(browser or {}).get('check_count'),'browser_pass_count':(browser or {}).get('pass_count'),
    ...
    for name in ['PATCH_EXECUTION_RECEIPT.json','EXACT_SOURCE_BINDING.json','BOOT_DIAGNOSTIC.json','EXACT_MONOLITH_BROWSER_COURT.json','PAGES_METADATA.json','PUBLIC_HOST_FETCH.json','FINAL_PRELAUNCH_GATE.json']:
        p=art/name
