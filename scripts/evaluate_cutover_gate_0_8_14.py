...
def main():
    ...
    lease=load('dynamic_lease_expiry_evidence.json')
    ...
    gates={
      ...
      'dynamic_persisted_lease_expiry_repetition':(
        lease.get('result')=='PASS_DYNAMIC_PERSISTED_LEASE_EXPIRY_REPEATED'
        and lease.get('fixed_sleep_used') is False
        and lease.get('iteration_count')==4
        and lease.get('passed_iterations')==4
        and lease.get('total_posts')==8
        and lease.get('all_payload_pairs_identical') is True
        and lease.get('all_final_generations_two') is True
      ),
    ...
    blocking_names=['evidence_lineage_reconciliation','exact_stored_raw_custody_replay','controlled_staging_browser_policy_matrix','dynamic_persisted_lease_expiry_repetition','physical_device_walkthrough','owner_acceptance_walkthrough']
    ...
    report={
      'schema_version':'CR0814-CONTROLLED-STAGING-CUTOVER-GATE-7',
      'gates':gates,
      'blocking_gates':blocking,
      'nonblocking_failures':nonblocking,
      'staging_candidate_available':gates['deterministic_staging_bundle'],
      'supported_active_profiles':['chromium-desktop','chromium-android-emulation'],
      'fail_closed_profiles':['firefox-desktop','firefox-compact-viewport','webkit-desktop','webkit-iphone-emulation','unknown-engines'],
      'lease_expiry_certification':'PERSISTED_EXPIRES_AT_PLUS_250MS_NEW_PAGE_TAKEOVER_4_ITERATIONS',
      'fixed_delay_lease_test_used_for_gate':False,
      'physical_device_certified':False,
      'production_cutover':cutover,
      'automatic_merge_authorized':False,
      'result':f'CONTROLLED_STAGING_CANDIDATE_PASS_PRODUCTION_CUTOVER_{cutover}'
    }
