...
def erratum_valid():
    ...
    lower=text.lower()
    return all((token.lower() in lower) for token in required)
def main():
    ...
    gates={
      ...
      'chromium_and_webkit_active_execution':matrix.get('active_execution_passed_cells')==4,
      'firefox_fail_closed_policy':matrix.get('fail_closed_passed_cells')==2 and matrix.get('firefox_active_execution_certified') is False,
      'physical_device_walkthrough':False,'owner_acceptance_walkthrough':False,
    ...
    report={'schema_version':'CR0814-CONTROLLED-STAGING-CUTOVER-GATE-4','gates':gates,'blocking_gates':blocking,'nonblocking_failures':nonblocking,'staging_candidate_available':gates['deterministic_staging_bundle'],'supported_active_engines':['chromium','webkit'],'fail_closed_engines':['firefox'],'production_cutover':cutover,'automatic_merge_authorized':False,'result':f'CONTROLLED_STAGING_CANDIDATE_PASS_PRODUCTION_CUTOVER_{cutover}'}
