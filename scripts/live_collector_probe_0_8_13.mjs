...
async function post(nonce){
  ...
  const fields={payload:collectorPayloadText,encoding:'json',study_id:'CUBE-REV-0.7.12',collector_id:'CUBE-REV-0712-MAIN',protocol_version:'receipt-v2',session_id:snapshot.session_id,version:'0.7.12',checksum_fnv1a32:checksum,original_bytes:Buffer.byteLength(collectorPayloadText),submission_nonce:nonce};
...
const evidence={
  schema_version:'CR0813-LIVE-COLLECTOR-EVIDENCE-1',started_at:startedAt,completed_at:new Date().toISOString(),
  endpoint_origin:new URL(endpoint).origin,endpoint_sha256:crypto.createHash('sha256').update(endpoint).digest('hex'),
  health:{ok:h.ok,collector_id:h.collector_id,protocol_version:h.protocol_version,expected_version:h.expected_version,deployment_id:h.deployment_id||null,receipt_confirmation_available:h.receipt_confirmation_available===true},
  collector_envelope:{schema_version:'CR0813-COLLECTOR-COMPATIBILITY-ENVELOPE-1',project:collectorEnvelope.project,version:collectorEnvelope.version,session_id:collectorEnvelope.session_id,sha256:collectorPayloadSha256,checksum_fnv1a32:a.fields.checksum_fnv1a32,bytes:Buffer.byteLength(collectorPayloadText)},
  snapshot:{session_id:snapshot.session_id,schema_version:snapshot.schema_version,version:snapshot.version,response_count:snapshot.responses.length,sha256:snapshotSha256},
  deliveries:[{nonce:nonceA,status:ra.status,receipt_code:ra.receipt_code||null,file_name:ra.file_name||null,polls:ra.polls},{nonce:nonceB,status:rb.status,receipt_code:rb.receipt_code||null,file_name:rb.file_name||null,polls:rb.polls}],
  result:'PASS_LIVE_COLLECTOR_TWO_NONCE_DEDUP'
};
