...
CORE=[
 ...
 'research/CUBE_REV_0.8.13_ERRATUM_FROM_0.8.14.md','research/CUBE_REV_0.8.14_ARCHIVAL_LIVE_WORKFLOW_EVIDENCE.json','research/CUBE_REV_0.8.14_BROWSER_SUPPORT_POLICY.json',
...
def main()->None:
    ...
    provenance={
      'schema_version':'CR0814-CONTROLLED-STAGING-PROVENANCE-5','project':'CUBE-REV','research_version':'0.8.14',
      ...
      'governing_browser_policy':'research/CUBE_REV_0.8.14_BROWSER_SUPPORT_POLICY.json',
      'browser_policy':{
    ...
    result={'schema_version':'CR0814-STAGING-BUILD-RESULT-5','candidate_fingerprint_sha256':provenance['candidate_fingerprint_sha256'],'zip_sha256':sha(ZIP.read_bytes()),'zip_bytes':ZIP.stat().st_size,'file_count':len([p for p in OUT.rglob('*') if p.is_file()]),'production_default_entry_modified':False,'collector_modified':False,'active_policy':['chromium_desktop_and_android_emulation'],'fail_closed_policy':['firefox_all_profiles','webkit_all_profiles','unknown_engines'],'physical_device_certified':False,'governing_browser_policy_included':True,'result':'PASS_DETERMINISTIC_STAGING_CANDIDATE_BUILD'}
    (ROOT/'artifacts/0.8.14/staging_build_result.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
    print(f"CR0814_STAGING_BUILD_PASS files={result['file_count']} zip_sha256={result['zip_sha256']} fingerprint={result['candidate_fingerprint_sha256']} active=CHROMIUM_ONLY policy_included=true")
