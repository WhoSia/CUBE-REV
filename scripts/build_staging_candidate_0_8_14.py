...
CORE=[
 'participant-cognitive-mode-0.8.13.html','collector-config.js','js/collector-client.js',
...
def main()->None:
    ...
    for rel in CORE:
        ...
        target_rel='index.html' if rel=='participant-cognitive-mode-0.8.13.html' else rel
    ...
    provenance={
      'schema_version':'CR0814-CONTROLLED-STAGING-PROVENANCE-1','project':'CUBE-REV','research_version':'0.8.14',
      ...
      'candidate_entry':'index.html','candidate_runtime_version':'CUBE-REV 0.8.13','deployment_authorized':False,
      'rollback_target':BASE_COMMIT,'files':sorted(files,key=lambda x:x['staging_path'])
    }
    ...
    rollback={'schema_version':'CR0814-STAGING-ROLLBACK-PLAN-1','rollback_target_commit':BASE_COMMIT,'rollback_action':'remove_staging_route_or_restore_parent_branch_bytes','production_index_untouched':True,'collector_untouched':True,'automatic_cutover_forbidden':True}
    ...
    result={'schema_version':'CR0814-STAGING-BUILD-RESULT-1','candidate_fingerprint_sha256':provenance['candidate_fingerprint_sha256'],'zip_sha256':sha(ZIP.read_bytes()),'zip_bytes':ZIP.stat().st_size,'file_count':len([p for p in OUT.rglob('*') if p.is_file()]),'production_default_entry_modified':False,'collector_modified':False,'result':'PASS_DETERMINISTIC_STAGING_CANDIDATE_BUILD'}
    (ROOT/'artifacts/0.8.14/staging_build_result.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
    print(f"CR0814_STAGING_BUILD_PASS files={result['file_count']} zip_sha256={result['zip_sha256']} fingerprint={result['candidate_fingerprint_sha256']}")
