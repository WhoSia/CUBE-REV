...
const LOCK_NAME='cube-rev-session-write-0812-exclusive-v1';
const JOURNAL_KEY='cube-rev-session-write-0812-journal-v1';
const CONFLICT_KEY='cube-rev-session-write-0812-conflicts-v1';
const SOURCE_0811='cube-rev-cognitive-mode-0811-v1';
const LEGACY_ROUTE='participant-cognitive-mode-0.8.11.html';
...
function readState(o){
  const raw=o.storage.getItem(o.m12.STORAGE_KEY);
  if(!raw)return {state:null,raw:null,invalid:false};
  const x=parse(raw);
  return {state:x,raw,invalid:!x||!o.m12.valid(x,o.config,o.binding)};
}
...
function commit(o,current,op,type,mutate,outcome='APPLIED'){
  ...
  const next=o.m12.seal(body);
  if(!o.m12.valid(next,o.config,o.binding))throw new Error(`CAS_CANDIDATE_INVALID:${type}`);
  o.storage.setItem(o.m12.STORAGE_KEY,JSON.stringify(next));
  const verified=verifyStored(o,next);
  writeJournal(o,{schema_version:'CR0812-ACTIVE-WRITE-JOURNAL-1',session_id:verified.session_id,mutation_id:op.mutation_id,type,from_revision:current.revision,to_revision:verified.revision,outcome,committed_at:at,integrity:verified.integrity});
...
function responseEquivalent(a,b){
  return !!a&&a.stimulus_id===b.stimulus_id&&a.choice_display===b.choice_display&&a.choice_code===b.choice_code&&a.latency_ms===b.latency_ms;
}
function conflictEvidence(o,current,op,clean,kind){
  const e={schema_version:'CR0812-WRITE-CONFLICT-1',kind,detected_at:o.now(),session_id:current.session_id,current_revision:current.revision,expected_revision:op.expected_revision,expected_position:op.expected_position,mutation_id:op.mutation_id,attempted_response:clean,stored_response:Number.isInteger(op.expected_position)?current.responses[op.expected_position]||null:null};
...
function applyLocked(o,op){
  ...
  switch(op.type){
    ...
    case 'TELEMETRY':{
      ...
      return commit(o,current,op,'TELEMETRY',x=>{
        x.telemetry=[...x.telemetry,{event_id:op.event_id,type:String(op.event_type||'EVENT').slice(0,100),at:op.at||o.now(),data:clone(op.data||{})}];return x;
    ...
    case 'POST_TASK':{
      ...
      return commit(o,current,op,'POST_TASK',x=>{
        ...
        x.telemetry=[...x.telemetry,{event_id:`${op.mutation_id}:telemetry`,type:'POST_TASK_SAVED',at:o.now(),data:{confidence:clean.confidence,deliberate_strategy_change:clean.deliberate_strategy_change}}];
        return x;
      },'POST_TASK_APPLIED');
    }
    case 'SEAL_SNAPSHOT':{
      if(current.submission_snapshot){
        if(!o.m12.snapshotValid(current))return reject('SUBMISSION_SNAPSHOT_INVALID',current,{blocking:true});
      ...
      return commit(o,current,op,'SEAL_SNAPSHOT',x=>{
        ...
        const snap=o.m12.scientificEnvelope(x),hash=o.m12.checksum(snap);
        x.submission_snapshot=snap;x.submission_snapshot_hash=hash;x.snapshot_sealed_at=sealingAt;
        x.submission_control={...x.submission_control,retry_id:`CR0812-RETRY-${o.m12.checksumText(x.session_id+hash)}`};
        return x;
      },'SNAPSHOT_SEALED');
    }
    case 'CLAIM_SUBMISSION':{
      if(current.status==='SUBMITTED')return {ok:true,action:'ALREADY_SUBMITTED',state:current,mutated:false};
      if(!current.submission_snapshot||!o.m12.snapshotValid(current))return reject('SNAPSHOT_NOT_READY',current);
      ...
      return commit(o,current,op,'CLAIM_SUBMISSION',x=>{
        ...
        x.telemetry=[...x.telemetry,{event_id:`${op.mutation_id}:telemetry`,type:'SUBMISSION_LEASE_CLAIMED',at:now,data:{lease_generation:generation,retry_id:x.submission_control.retry_id}}];
        return x;
    ...
    case 'SUBMISSION_META':{
      ...
      return commit(o,current,op,'SUBMISSION_META',x=>{
        x.submission_control={...x.submission_control,collector_meta:{...(x.submission_control.collector_meta||{}),...meta},collector_events:Array.isArray(op.collector_events)?clone(op.collector_events).slice(-64):x.submission_control.collector_events};
        return x;
      },'SUBMISSION_META_MERGED');
    }
    case 'RELEASE_SUBMISSION':{
      if(current.status==='SUBMITTED')return {ok:true,action:'ALREADY_SUBMITTED',state:current,mutated:false};
      if(current.submission_control.lease_token!==op.lease_token)return reject('STALE_SUBMISSION_LEASE',current);
      return commit(o,current,op,'RELEASE_SUBMISSION',x=>{
        const at=o.now();x.submission_control={...x.submission_control,lease_token:null,lease_owner:null,lease_expires_at:null,last_error:String(op.error||'').slice(0,500)};
        x.telemetry=[...x.telemetry,{event_id:`${op.mutation_id}:telemetry`,type:'SUBMISSION_LEASE_RELEASED',at,data:{failed:!!op.error}}];return x;
      },'SUBMISSION_LEASE_RELEASED');
    }
    case 'CONFIRM_SUBMISSION':{
      if(current.status==='SUBMITTED')return {ok:true,action:'ALREADY_SUBMITTED',state:current,mutated:false};
      if(current.submission_control.lease_token!==op.lease_token)return reject('STALE_SUBMISSION_LEASE',current);
      return commit(o,current,op,'CONFIRM_SUBMISSION',x=>{
        const at=o.now();x.status='SUBMITTED';x.submitted_at=at;x.submission_receipt=clone(op.receipt||null);
        x.submission_control={...x.submission_control,lease_token:null,lease_owner:null,lease_expires_at:null,receipt:clone(op.receipt||null),last_error:null};
        x.telemetry=[...x.telemetry,{event_id:`${op.mutation_id}:telemetry`,type:'SUBMISSION_CONFIRMED',at,data:{retry_id:x.submission_control.retry_id}}];return x;
...
function quarantineInvalid(o,raw){
  o.storage.setItem(o.m12.QUARANTINE_KEY,raw);o.storage.removeItem(o.m12.STORAGE_KEY);
}
...
async function loadOrInitializeActive(o){
  ...
  const parent=await o.atomic11.loadOrInitializeAtomic(o.atomic11Deps);
  ...
  return o.lockManager.request(LOCK_NAME,{mode:'exclusive'},()=>{
    ...
    const raw=o.storage.getItem(SOURCE_0811),x=parse(raw);
    if(!x||!o.m11.valid(x,o.parentConfig,o.parentBinding))return {action:'PARENT_STATE_INVALID',state:null,blocking:true};
    if(x.submission_snapshot||x.status==='SUBMITTED')return {action:'LEGACY_SEALED_RETRY',state:null,legacy_route:LEGACY_ROUTE};
    const target=o.m12.from0811(o.config,{binding:o.binding,parentConfig:o.parentConfig,parentBinding:o.parentBinding,parentStorageKey:SOURCE_0811,m11:o.m11,now:o.now},x);
    o.storage.setItem(o.m12.STORAGE_KEY,JSON.stringify(target));
    const verified=verifyStored(o,target);
    writeJournal(o,{schema_version:'CR0812-ACTIVE-WRITE-JOURNAL-1',session_id:verified.session_id,mutation_id:verified.mutation_history[0].mutation_id,type:'ACTIVE_SESSION_UPGRADE',from_revision:0,to_revision:1,outcome:'UPGRADED_FROM_0811',committed_at:verified.updated_at,integrity:verified.integrity});
    return {action:'UPGRADED_FROM_0811',state:verified,resumed:false};
  });
}
function makeIdFactory(cryptoObj){
  return prefix=>{
    const a=new Uint32Array(2);cryptoObj.getRandomValues(a);
    return `CR0812-${String(prefix||'M').toUpperCase()}-${Array.from(a,x=>x.toString(16).padStart(8,'0')).join('')}`;
  };
}

const api={LOCK_NAME,JOURNAL_KEY,CONFLICT_KEY,SOURCE_0811,LEGACY_ROUTE,clone,parse,hasLockManager,mutationSeen,telemetrySeen,readState,writeConflict,applyLocked,applyOperation,loadOrInitializeActive,makeIdFactory};
if(typeof module!=='undefined'&&module.exports)module.exports=api;
global.CUBE_REV_ACTIVE_SESSION_CAS_0812=api;
