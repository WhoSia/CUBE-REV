...
function validateSource(raw,version,o){
  ...
  if(!source)return {valid:false,source:null};
  if(version==='0.8.8')return {valid:o.m088.valid(source,o.config088),source};
  return {valid:o.m089.valid(source,o.config089),source};
}
function chooseSource(o){
  ...
  const invalid=[];
  if(a&&!a.valid)invalid.push({source_storage_key:SOURCE_088,raw:raw088});
  if(b&&!b.valid)invalid.push({source_storage_key:SOURCE_089,raw:raw089});
...
const api={VERSION,JOURNAL_KEY,MIGRATION_QUARANTINE_KEY,ARCHIVE_PREFIX,SOURCE_088,SOURCE_089,parse,fingerprint,bankIndex,legacyRoute,migrationId,quarantine,sealed,normalize088,normalize089,material,validateSource,chooseSource,migrateOne,loadOrMigrate};
